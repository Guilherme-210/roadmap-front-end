...
Todos os exercícios compartilham um único aplicativo Next.js em `next-lab/`, ao lado do cofre `obsidian/`. A raiz é `F:\Obsidian\roadmap-front-end`. Execute os comandos npm/pnpm dentro de `next-lab/` ou na raiz, onde ficam package.json e src/app. Organize novos exercícios do aplicativo em `next-lab/app/exercicios/<tecnologia-ou-modulo>/<slug>/`, sem criar aplicativos ou repositórios Git aninhados. 

No cofre, antes de gerar qualquer exercício, analise a complexidade e amplitude do subnível para definir se ele necessita de 1 ou mais exercícios. Caso o conteúdo do subnível exija mais de uma prática para cobrir bem os conceitos, devem ser gerados exercícios adicionais condizentes com aquele subnível. Cada exercício deve obrigatoriamente ser armazenado dentro de uma subpasta em `obsidian/03 - Prática/` com o nome exato do nível a que pertence (exemplo: `obsidian/03 - Prática/Nível 07 - React/React - Exercício 01 - Cartão de estudo.md`). Cada exercício deve seguir o padrão do `obsidian/90 - Modelos/Modelo - Exercício.md` e estar indexado em `obsidian/03 - Prática/Prática.md`. 

Se for solicitado um novo exercício para um subnível que já possui um exercício criado, o arquivo antigo NUNCA pode ser modificado ou apagado. O novo exercício deve ser gerado como uma nova nota no mesmo diretório do nível, alterando apenas o final do nome (exemplo: `React - Exercício 01 - {novo nome da proposta}`). Código e notas devem ser versionados juntos a partir da raiz. Preserve o lockfile e use as verificações documentadas em next-lab/README.md.

O ambiente usa Next.js 16, React 19, TypeScript, Tailwind CSS, shadcn/ui, TanStack Query, React Hook Form e Zod, além de ferramentas de testes e qualidade. Acrescente dependências futuras conforme uma necessidade concreta do exercício. A nota `obsidian/02 - Aprendizados/Ambiente de prática - Next.js e dependências.md` documenta as escolhas. A instalação do ambiente não comprova domínio de nenhum nível e não autoriza resolver exercícios pelo usuário.
